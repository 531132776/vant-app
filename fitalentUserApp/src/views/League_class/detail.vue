<template>
    <div class="storeDetails pt15">
        ...
        <div class="addres pr_pl15">
            <ul>
                <li>
                    ...
                    <span style="color:#b2b6bc;">累计参课人数{{leagurList.allPeopleNum}}人
                    </span>
                </li>
            </ul>
        </div>
        ...
        <!-- 协议 -->
        <div class="class_introduce pr_pl15" style="overflow:hidden">
                <div class="careful_Agreement">注意事项</div>
                <div class="Agreement"><em>成功预约本课程即表示已阅读并同意</em> <br/>
                    <router-link to="/leagueClassAgreement" style="color:#3A5891">《健康传奇团课协议》</router-link>
                </div>
            </div>
        <!-- 课程介绍 -->
        ...
        <div class="appointment pr_pl15">
            ...
            <van-button class="appointment_btn" v-if="status == 0"  @click="appointment()" type="primary">立即预约</van-button>
            <van-button class="gray" v-if="status == 1"  type="primary">已预约</van-button>
            ...
            <van-button class="gray" v-if="status == 5"  type="primary">上课中</van-button>
        </div>
    </div>
</template>
        ...
        methods: {
            ...
            appointment() {
                Dialog.confirm({
                    ...
                    UpateAppointmentStatus({
                        status: 1,
                        clubId: this.leagurList.clubId,
                        brand: this.leagurList.brand,
                        courseId: this.leagurList.teamClassBusinessId,
                        userId: this.userId,
                    }).then(res => {
                        if(res.data.code == 2000){
                            this.status = res.data.obj
                            this.init()
                            Dialog.confirm({
                            message: '预约成功',
