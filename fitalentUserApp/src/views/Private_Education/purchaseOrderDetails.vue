        ...
        <div class="address_info pr_pl15">
          <div class="content_text pt_pb15">
            <ul @click="popup">
              ...
              <li>
                <dt class="text_gray">
                  <span v-if="couponListLength && this.couponList.length>0">{{couponName}}</span>
          ...
          <div class="content_text pt_pb15">
            <ul>
              ...
              <li class="price_font" v-if="status==0">
                ...
                <em>{{Number(experienceTotalPrice).toFixed(2)}}</em>
              </li>
              <!-- 2是私教课 -->
              <li class="price_font" v-if="status==2">
                <em>¥</em>
                <em v-if="contrastStatus==0">{{Number(totalOnePrice).toFixed(2)}}</em>
                <em v-if="contrastStatus==1">{{Number(totalTwoPrice).toFixed(2)}}</em>
                <!-- <em>/节</em> -->
              </li>
              <!-- 1是包月私教 -->
    ...
    <div class="Immediate_purchase">
      <div class="purchase_info">
        <ul>
          <li v-if="status==0">
            <dt>共计: ¥</dt>
            <dt>{{Number(experiencePrice).toFixed(2)}}</dt>
          </li>
          <li v-if="status==2">
            <dt>共计: ¥</dt>
          ...
          <!-- 私教课支付 -->
          <li v-if="status==2" >
            <van-button type="default" @click.stop="DesignatedcoursesPayment()" :disabled="isDisable">去支付</van-button>
          </li>
          <!-- 包月支付 -->
          <li v-if="status==1" >
            <!-- <span>去支付</span> -->
            <van-button type="default" @click.stop="monthlyNmbcoursesPayment()" :disabled="isDisable">去支付</van-button>
          </li>
          <!-- 私教体验课支付 -->
          <li v-if="status==0" >
            <!-- <span>去支付</span> -->
            <van-button type="default" @click.stop="PrivateeducationPayment()" :disabled="isDisable">去支付</van-button>
          </li>
        </ul>
      </div>
    </div>
...
export default {
                ...
                if(res.data.code == 2000){
                  ...
                  //   console.log("私教课", this.privateOne);
                    ...
                    if(this.contrastStatus == 0){
                        ...
                        this.totalPrice = this.nmb * this.privateCourse.onePrice - this.afterCoupon;
                        this.totalOnePrice = this.nmb * this.privateCourse.onePrice;
                        console.log(this.totalOnePrice,'------>>0')
                        this.GetCouponList(
                        ...
                        this.totalPrice = this.nmb * this.privateCourse.twoPrice - this.afterCoupon;
                        this.totalTwoPrice = this.nmb * this.privateCourse.twoPrice;
                        console.log(this.totalTwoPrice,'------>>1')
                        this.GetCouponList(
                            this.$route.query.courseType,
                            this.userId,
                            this.nmb,
      ...
      //获取包月私教详情
            ...
            privateMonthCourse(params).then(res=>{
                ...
                if(res.data.code == 2000){
                    ...
                    // console.log()
                    var total = this.monthlyNmb * this.MonthTotalPrice;
                    this.GetCouponList(
                        this.$route.query.courseType,
                        this.userId,
                        this.monthlyNmb,
  ...
  methods: {
    GetCouponList(type, userId, goodsNum, amount) {
      GetCouponRecordList({
        ...
        res.data.obj.list.length > 0
          ...
          : "";
        // res.data.obj.list.length == 0 ? this.afterCoupon == 0 : this.$set(this,'afterCoupon',this.couponList[0].discountValue);
        if(this.couponCount == 0){
          this.afterCoupon = 0;
        }
        // console.log(res.data.obj.list[0].checkStatus,'初始勾选')
        if(this.couponList.length>0){
            this.couponName = this.couponList[0].couponName;
            ...
            if(this.couponList[0].couponType == 0){
                ...
                if(this.contrastStatus == 0){
                    this.totalPrice = amount-this.afterCoupon;
                }else if(this.contrastStatus == 1){
                    // alert(1111)
                    this.totalPrice = amount-this.afterCoupon;
                }
                ...
                if(this.status == 1){
                    ...
                    // alert('包月');
                    // this.$set(this,'afterCoupon',this.couponList[0].discountValue);
                      this.monthlyTotalPrice = amount - this.afterCoupon
    ...
    aaa(item) {
      ...
      if (!item.checkStatus) {
        console.log(item.checkStatus,'选项')
        ...
        this.couponList.map(function(res) {
          ...
          console.log(res, "尼玛");
          _this.$set(_this,'couponName',res.couponName);
          ...
          if(res.couponType == 0){
            //   alert('代金券')
              if(_this.status == 2){
                  //私教课
                //   alert('私教')
                  // _this.nmb = _this.privateCourse.lowestSection;
                  // alert(_this.nmb)
                  _this.$set(_this,'afterCoupon',res.discountValue);
                  if(_this.contrastStatus == 0){
                      _this.totalPrice = (_this.privateCourse.onePrice * _this.nmb)-_this.afterCoupon;
                  }else if(_this.contrastStatus == 1){
                    //   alert('true')
                      _this.totalPrice = (_this.privateCourse.twoPrice * _this.nmb)-_this.afterCoupon;
                  }
            ...
            //   alert('私教体验课');
              ...
              if(_this.status == 1){
                  ...
                  // alert('包月')
                _this.$set(_this,'afterCoupon',res.discountValue);
                _this.monthlyTotalPrice = (_this.educationsectorDetails.price*_this.monthlyNmb) - _this.afterCoupon
              }
            ...
            //   alert('免费券')
              ...
              if(_this.status == 2){
                ...
                //   alert('私教课')
                    ...
                    if(_this.contrastStatus == 0){
                        _this.afterCoupon = _this.privateCourse.onePrice * _this.nmb;
                        _this.totalPrice = (_this.privateCourse.onePrice * _this.nmb)-_this.afterCoupon
                    }else if(_this.contrastStatus == 1){
                        _this.afterCoupon = _this.privateCourse.twoPrice * _this.nmb;
                        _this.totalPrice = (_this.privateCourse.twoPrice * _his.nmb)-_this.afterCoupon
                    }
    ...
    activeItem(v, index) {
      ...
      if (index == 0) {
        ...
        this.totalPrice =
          this.privateCourse.onePrice * this.nmb-this.afterCoupon;
        ...
        this.nmb = this.privateCourse.lowestSection;
        this.totalPrice =
          this.privateCourse.twoPrice * this.nmb-this.afterCoupon;
    ...
    reduce() {
      ...
      if (this.contrastStatus == 0 && this.nmb >= this.privateCourse.lowestSection) {
        ...
        //  alert((this.totalTwoPrice).toFixed(2));
        this.GetCouponList(
          this.privateOne.courseType,
          this.privateOne.userId,
          this.nmb,
